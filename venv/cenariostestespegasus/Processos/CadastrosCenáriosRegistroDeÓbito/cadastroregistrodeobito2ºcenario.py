...
doc.add_paragraph("Cadastro de Registro de Óbito – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    ...
    nome_arquivo = f"relatorio_obito_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
# Data do falecimento
    ...
    driver, wait, "input[ref='100159']", data_velorio,))
        ...
        safe_action(doc, "Cancelando cadastro", clicar_elemento_robusto(
            '#fmod_23 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel'
        ))
