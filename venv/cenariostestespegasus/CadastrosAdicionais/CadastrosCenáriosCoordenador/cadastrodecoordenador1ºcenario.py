...
doc.add_paragraph("Cadastro de Coordenador – Cenário 1: Preenchimento completo e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_coordenador_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Salvando dados da pessoa", lambda: (
    ...
    safe_action(doc, "Salvando cadastro do coordenador", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10017 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btsave"))).click(),
        time.sleep(1)
    ))
