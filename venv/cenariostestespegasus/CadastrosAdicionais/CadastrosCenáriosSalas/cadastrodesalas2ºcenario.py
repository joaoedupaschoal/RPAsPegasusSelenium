...
campo_pesquisa.send_keys("Salas", Keys.ENTER)



time.sleep(3)

cadastrar = driver.find_element(By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span")
cadastrar.click()
...
campo_nome = WebDriverWait(driver, 10).until(
    EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(2) > input"))
)
campo_nome.send_keys('SALA TESTE SELENIUM AUTOMATIZADO')

campo_cor = WebDriverWait(driver, 10).until(
    EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(3) > input"))
)
campo_cor.send_keys('Red', Keys.TAB)



campo_tempo_intervalo =WebDriverWait(driver, 10).until(
    EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(4) > input"))
    )
campo_tempo_intervalo.send_keys(fake.random_int(min=10, max=500))


open_lov = WebDriverWait(driver, 10).until(
    EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(6) > div > a"))
)
open_lov.click()
...
campo_pesquisa_cemi.send_keys('CEMITÉRIO JOÃO TESTE ROBO', Keys.ENTER)

time.sleep(2)

# Espera até que a linha com o cemitério específico esteja visível
cemiterio = WebDriverWait(driver, 10).until(
    EC.element_to_be_clickable((By.XPATH, "//td[contains(text(), 'CEMITÉRIO JOÃO TESTE ROBO')]"))
)
...
Select(driver.find_element(By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(5) > select")).select_by_visible_text("ALUGUEL CAPELA PREMIUM")


# Clique no botão "Cancelar"

wait = WebDriverWait(driver, 10)  # Espera até 10 segundos
element = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10042 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel")))
element.click()
...
# Fechar modal
X = driver.find_element(By.CSS_SELECTOR, "#fmod_10042 > div.wdTop.ui-draggable-handle > div.wdClose > a")
X.click()
time.sleep(1)
