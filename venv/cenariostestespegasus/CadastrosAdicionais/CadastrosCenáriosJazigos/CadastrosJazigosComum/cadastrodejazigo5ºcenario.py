...
doc.add_paragraph("Cadastro de Jazigos – Cenário 5: Nesse teste, o robô preencherá todos os dados, excedendo o limite máximo de Jazigos de uma Quadra e clicará em Salvar")
...
# Função alternativa para lidar com modal de LOV
def aguardar_e_interagir_com_lov_quadra(driver, timeout=30):
    """
    Função específica para aguardar e interagir com o modal de LOV da quadra
    """
    try:
        # Aguarda o modal aparecer
        WebDriverWait(driver, timeout).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".modal, .modalHolder"))
        )
        log(doc, "✅ Modal LOV detectado")
        
        # Aguarda a tabela dentro do modal carregar
        WebDriverWait(driver, timeout).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".modal table, .modalHolder table"))
        )
        log(doc, "✅ Tabela dentro do modal carregada")
        ...
        log(doc, "❌ Timeout: Modal LOV não apareceu ou tabela não carregou")
        return False
    except Exception as e:
        log(doc, f"❌ Erro ao aguardar modal LOV: {e}")
        return False



# Substituir a seção de seleção de quadra no script principal por esta versão melhorada:
...
def selecionar_item_tabela_melhorado_quadra(driver, texto_busca, timeout=20):
    ...
    estrategias = [
        ...
        "//tr[@ref='31100174' and @role='row' and contains(@class, 'odd') and @style='cursor: pointer;'][td[1]='TESTE QUADRA LOTADA' and td[2]='1' and td[3]='1' and td[4]='1' and td[5]='CEMITÉRIO HERMAN DESCANSO']"
    ]
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_jazigos_cenario_5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
try:
    ...
    safe_action_enhanced(driver, doc, "Inserindo nome da Quadra", 
        lambda: safe_send_keys_enhanced(driver, "body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div.formCol.divPesquisa > div:nth-child(2) > input", 'TESTE QUADRA LOTADA'))
    ...
    safe_action_enhanced(driver, doc, "Selecionando Quadra",
        lambda: selecionar_item_tabela_melhorado_quadra(driver, "TESTE QUADRA LOTADA"), max_tentativas=5)
