...
doc.add_paragraph("Cadastro de Escala Motorista – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_escala_motorista_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Acessando aba Endereços", lambda: driver.find_element(
        By.CSS_SELECTOR, "#cg_1 > div.wdTelas > div > div.categorias.overflow.overflowY > ul > li.li_enderecos > a"
    ).click())

    safe_action(doc, "Marcando endereço principal", lambda: (
        time.sleep(3),
    ...
    safe_action(doc, "Preenchendo data de admissão", lambda: (
        ...
        driver.find_element(By.CSS_SELECTOR, 'input.dataAdmissao').clear(),
    ...
    safe_action(doc, "Preenchendo data início da escala", lambda: (
        ...
        driver.find_element(By.CSS_SELECTOR, 'input.dataInicio').clear(),
    ...
    safe_action(doc, "Preenchendo data fim da escala", lambda: (
        ...
        driver.find_element(By.CSS_SELECTOR, 'input.dataFim').clear(),
    ...
    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_3000002 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroEscalaMotorista > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())

    time.sleep(1)
    encontrar_mensagem_alerta()

    safe_action(doc, "Fechando modal", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_3000002 > div.wdTop.ui-draggable-handle > div > a"
    ).click())
