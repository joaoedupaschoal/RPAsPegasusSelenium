...
doc.add_paragraph("Cadastro de Manutenção de Veículos – Cenário 3: Preenchimento dos campos obrigatórios e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_manutencao_veiculos_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Selecionando o Veículo", abrir_modal_e_selecionar(
        ...
        "//td[contains(text(), 'S10')]"
    ))    

    # Preenchendo dados da manutenção
    safe_action(doc, "Preenchendo KM Verificado", lambda: (
        preencher_campo_com_retry(driver, wait, "#fmod_10062 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroManutencaoVeiculo > div.catWrapper > div > div > div.groupHolder.clearfix.grupo_grupoDadosGerais > div > div > div:nth-child(4) > input", dados['km_verificado'])
    ))

    safe_action(doc, "Selecionando Oficina", abrir_modal_e_selecionar(
        "#fmod_10062 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroManutencaoVeiculo > div.catWrapper > div > div > div.groupHolder.clearfix.grupo_grupoDadosManutencao > div > div > div:nth-child(1) > div > a",
        "#txtPesquisa",
        "POSTO IPIRANGA",
        "body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(2) > a",
        "//td[contains(text(), 'POSTO IPIRANGA')]"
    ))    

    safe_action(doc, "Selecionando Tipo manutenção", selecionar_opcao(
        "#fmod_10062 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroManutencaoVeiculo > div.catWrapper > div > div > div.groupHolder.clearfix.grupo_grupoDadosManutencao > div > div > div:nth-child(2) > select",
        "Preventiva"
    ))


    safe_action(doc, "Preenchendo Descrição do Serviço", lambda: (
        preencher_campo_com_retry(driver, wait, "#fmod_10062 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroManutencaoVeiculo > div.catWrapper > div > div > div.groupHolder.clearfix.grupo_grupoDadosManutencao > div > div > div:nth-child(5) > textarea", 'A MANUTENÇÃO FOI BEM SUCEDIDA, O CABEÇOTE DO CARRO FOI REFEITO, VEÍCULO ESTÁ EM PERFEITO ESTADO PARA USO, PNEU CALIBRADO E TANQUE CHEIO.')
    ))
