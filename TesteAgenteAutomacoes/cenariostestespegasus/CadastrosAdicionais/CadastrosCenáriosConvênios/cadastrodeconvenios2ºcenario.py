...
doc.add_paragraph("Cadastro de Convênios – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_convenio_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preechendo o Nome do Convênio", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10065 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroConvenio > div.catWrapper > div > div > div > div > div > div:nth-child(2) > input"))
        ).send_keys('TESTE CONVÊNIO SELENIUM AUTOMATIZADO')
    ))
    ...
    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10065 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroConvenio > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
