...
doc.add_paragraph("Cadastro de Tipo Venda – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_tipo_venda_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preechendo a Descrição do Tipo de Venda", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10019 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div > input"))
        ).send_keys('TESTE TIPO VENDA SELENIUM AUTOMATIZADO')
    ))



    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10019 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
