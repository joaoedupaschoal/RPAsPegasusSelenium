...
doc.add_paragraph("Cadastro de Formas de Pagamento MXM – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_formas_de_pagamento_mxm_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10070 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroFormasPagamentoMXM > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
