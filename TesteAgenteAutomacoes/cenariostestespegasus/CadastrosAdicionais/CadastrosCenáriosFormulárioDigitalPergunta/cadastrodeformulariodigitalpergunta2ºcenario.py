...
doc.add_paragraph("Cadastro de Formulário Digital Pergunta – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_formulario_digital_pergunta_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    for i, alternativa in enumerate(alternativas):
        safe_action(doc, f"Criando alternativa {i+1}: {alternativa}", criar_alternativa(alternativa, i+1))

    safe_action(doc, "Cancelando cadastro da pergunta", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10043 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
        time.sleep(1)
    ))
