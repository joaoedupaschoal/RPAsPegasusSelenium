...
doc.add_paragraph("Cadastro de Cor – Cenário 3: Preenchimento dos campos NÃO origatórios e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_cor_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Abrindo menu Cor", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("Cor"),
        wait.until(EC.visibility_of_element_located((By.XPATH, "/html/body/div[17]/ul/li[19]/a"))).click()
    
    ))

    safe_action(doc, "Clicando em Cadastrar", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10030 > div.wdTelas > div > ul > li:nth-child(1) > a > span"))).click()
    ))
    ...
    safe_action(doc, "Salvando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10030 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroCor > div.btnHolder > a.btModel.btGray.btsave"
    ).click())
