...
doc.add_paragraph("Cadastro de Formulário Digital – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_formulario_digital_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    ...
    def acao():
        ...
        if pergunta_data['alternativas']:
            for i, alternativa_texto in enumerate(pergunta_data['alternativas']):
                ...
                cancelar_alternativa = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#cg_10044 > div.wdTelas > div > div.btnHolder > a.btModel.btGray.btsave")))
                cancelar_alternativa.click()
                time.sleep(5)
        ...
        # Cancelar pergunta
        cancelar_pergunta = wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#cg_10043 > div.wdTelas > div > div.btnHolder > a.btModel.btGray.btsave")))
        cancelar_pergunta.click()
        time.sleep(5)
...
try:
    ...
    safe_action(doc, "Cancelando formulário digital", lambda: (
        time.sleep(5),
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10045 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
    ))
