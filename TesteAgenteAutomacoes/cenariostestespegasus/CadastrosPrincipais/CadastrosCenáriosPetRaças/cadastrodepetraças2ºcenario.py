...
doc.add_paragraph("Cadastro de PET - Raças – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_pet_racas_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Abrindo menu PET - Raças", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("PET - Raças", Keys.ENTER),
    ))
    ...
    safe_action(doc, "Preenchendo o Nome da Raça do PET", lambda: (
        time.sleep(2),
        ...
        .send_keys('TESTE RAÇA DO PET SELENIUM AUTOMATIZADO' + str(fake.random_int(min=1, max=1000)))
    ))

    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_200005 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
