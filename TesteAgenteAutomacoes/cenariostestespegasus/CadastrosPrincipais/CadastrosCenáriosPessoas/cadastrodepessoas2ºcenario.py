...
doc.add_paragraph("Cadastro de Pessoas – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_pessoas_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preenchendo Endereço - Método Retry", 
            lambda: preencher_campo_com_retry(driver, wait, seletor_campo, "15081115")
        )
    ...
    safe_action(doc, "Cancelando cadastro", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_1 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroPessoa > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
        time.sleep(1)
    ))
