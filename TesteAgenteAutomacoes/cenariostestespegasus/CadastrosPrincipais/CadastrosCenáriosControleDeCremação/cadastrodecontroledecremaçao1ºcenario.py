...
def main():
    ...
    doc.add_paragraph("Cadastro de Controle de Cremação - 1º Cenário.")
    doc.add_paragraph(f"🗓️ Data do teste: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
    doc.add_paragraph("Neste teste, o robô preencherá todos os campos e salvará o cadastro de um novo Controle de Cremação.")
    ...
    def finalizar_relatorio():
        ...
        doc_name = f"relatorio_controle_cremacao_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
        doc.save(doc_name)
