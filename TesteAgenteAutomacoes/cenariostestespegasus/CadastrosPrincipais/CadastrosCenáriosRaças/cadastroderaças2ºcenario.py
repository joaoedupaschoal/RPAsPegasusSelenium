...
doc.add_paragraph("Cadastro de Raças – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_racas_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    # Preenchendo dados da raça
    safe_action(doc, "Preenchendo Nome da Raça", lambda: (
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10081 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div > input")))
        .send_keys(nome_raca)
    ))

    safe_action(doc, "Cancelando cadastro", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10081 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
        time.sleep(1)
    ))
