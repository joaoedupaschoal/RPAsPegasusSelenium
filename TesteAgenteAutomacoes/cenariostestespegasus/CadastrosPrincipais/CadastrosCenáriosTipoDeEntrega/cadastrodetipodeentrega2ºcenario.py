...
doc.add_paragraph("Cadastro de Tipo de Entrega – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_tipo_entrega_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Abrindo menu Tipo de Entrega", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("Tipo de Entrega", Keys.ENTER),
    ))

    safe_action(doc, "Clicando em Cadastrar", lambda: (
        time.sleep(3),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10078 > div.wdTelas > div > ul > li:nth-child(1) > a > span"))).click()
    ))
    ...
    safe_action(doc, "Preenchendo o campo Descrição", lambda: (
        ...
        .send_keys('TESTE DESCRIÇÃO TIPO DE ENTREGA SELENIUM AUTOMATIZADO' + str(fake.random_int(min=1, max=1000)))
    ))

    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10078 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroTipoEntrega > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())

    safe_action(doc, "Fechando modal após cancelamento", lambda: wait.until(EC.element_to_be_clickable((
        By.CSS_SELECTOR, "#fmod_10078 > div.wdTop.ui-draggable-handle > div.wdClose > a"
        ))
    ).click())
