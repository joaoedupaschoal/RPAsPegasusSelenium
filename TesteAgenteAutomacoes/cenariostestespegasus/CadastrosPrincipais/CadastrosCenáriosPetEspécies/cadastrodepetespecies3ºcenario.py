...
doc.add_paragraph("Cadastro de PET - Espécies – Cenário 3: Preenchimento dos campos NÃO obrigatórios e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_pet_especies_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Abrindo menu PET - Espécies", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("PET - Espécies", Keys.ENTER),
    ))

    safe_action(doc, "Clicando em Cadastrar", lambda: (
        time.sleep(3),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_200004 > div.wdTelas > div > ul > li:nth-child(1) > a > span"))).click()
    ))

    time.sleep(2)  # Espera o modal abrir completamente

    safe_action(doc, "Salvando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_200004 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btsave"
    ).click())
