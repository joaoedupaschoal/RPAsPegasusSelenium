...
doc.add_paragraph("Cadastro de Reclamações – Cenário 1: Preenchimento completo e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_reclamacoes_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Salvando cadastro de pessoa", lambda: (
    ...
    safe_action(doc, "Salvando reclamação", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_200028 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btsave"))).click(),
        time.sleep(1)
    ))
