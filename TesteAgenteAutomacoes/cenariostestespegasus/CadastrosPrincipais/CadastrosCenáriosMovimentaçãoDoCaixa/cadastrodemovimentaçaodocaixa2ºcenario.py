...
doc.add_paragraph("Cadastro de Movimentação do Caixa – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_movimentacao_caixa_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preenchendo Data de Lançamento", 
                preencher_data_duplo("//input[@grupo='10019' and @ref='10020' and contains(@class, 'hasDatepicker mandatory fc')]", 
                              dados['data_lancamento']))

    safe_action(doc, "Preenchendo Valor", 
                preencher_campo("#fmod_10013 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(4) > input", 
                               f"{dados['valor']:.2f}".replace('.', ',')))

    safe_action(doc, "Selecionando Centro de Custo", 
                abrir_lov_e_selecionar("#fmod_10013 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(5) > div > div > a",
                                      "82.93.7260",
    ...
    safe_action(doc, "Selecionando Conta Débito", 
                abrir_lov_e_selecionar("#fmod_10013 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(11) > div > div > a",
                                      "782",
                                      "//tr[td[contains(text(), 'INTERNET VIRADOURO')]]//a[contains(@class, 'linkAlterar')]"))

    safe_action(doc, "Selecionando Conta Crédito", 
                abrir_lov_e_selecionar("#fmod_10013 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(12) > div > div > a",
                                      "748",
    ...
    safe_action(doc, "Clicando em Cancelar", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10013 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
        time.sleep(1)
    ))
