...
doc.add_paragraph("Cadastro de Contrato Pet – Cenário 2: Preenchimento completo e cancelamento.")
...
def cancelar_cadastro():
    seletor_css_cancelar = '#gsPet > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(1)'
    clicar_elemento_robusto(driver, wait, seletor_css_cancelar)
    time.sleep(6)
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_contrato_pet_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Cancelando cadastro", cancelar_cadastro)
