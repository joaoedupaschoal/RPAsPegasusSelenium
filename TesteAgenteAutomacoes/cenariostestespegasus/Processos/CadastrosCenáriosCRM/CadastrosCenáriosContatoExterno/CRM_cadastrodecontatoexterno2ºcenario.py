...
doc.add_paragraph("Cadastro de Contato Externo (CRM) – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_contato_externo_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando em Cadastrar Contato Externo", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCrm > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span'))).click()
        )

        time.sleep(2)

        safe_action(doc, "Avançando para aba: 'Cliente/Empresa'", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)')
        )

        safe_action(doc, "Selecionando Cliente", abrir_modal_e_selecionar_robusto(
        ...
        safe_action(doc, "Salvando Cliente", lambda:
            clicar_elemento_robusto(driver, wait, '#ui-id-2 > div:nth-child(10) > div:nth-child(5) > a')
        )

        safe_action(doc, "Avançando para aba: 'Pacotes'", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)')
        )

        safe_action(doc, "Selecionando Pacote", abrir_modal_e_selecionar_robusto(
            '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.stepPacote.step3 > div > div > div > div > a',
        ...
        safe_action(doc, "Avançando para aba: 'Itens'", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)')
        )

        safe_action(doc, "Selecionando Item", abrir_modal_e_selecionar_robusto(
            '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.stepItem.step4 > div.tableProdutos > div.tbHeader > ul > li:nth-child(1) > div > div > a',
            'body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(1) > input',
            'ABERTURA DE JAZIGO',
            'body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(2) > a',
            "//td[contains(text(), 'ABERTURA DE JAZIGO')]"
        ))

        safe_action(doc, "Avançando para aba: 'Tarefas'", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)')
        )

        safe_action(doc, "Selecionando Tipo de Tarefa", lambda:
            Select(wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.stepTarefa.step5 > div:nth-child(1) > div > select")))).select_by_visible_text("Ligação")
        )

        safe_action(doc, "Preechendo Assunto", lambda:
            wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.stepTarefa.step5 > div:nth-child(2) > div > textarea"))).send_keys("TESTE DE LIGAÇÃO AUTOMATIZADA - SELENIUM")
        )

        safe_action(doc, "Preenchendo Data Início", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "(//input[contains(@class,'hasDatepicker') and @type='text' and contains(@style,'130px')])[1]",
                data_inicio
            )
        )
        ...
        safe_action(doc, "Preenchendo Data Fim", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "(//input[contains(@class,'hasDatepicker') and @type='text' and contains(@style,'130px')])[2]",
                data_fim
    )
)
        ...
        safe_action(doc, "Cancelando cadastro", lambda:
            clicar_elemento_robusto(driver, wait, "#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(1)")
        )



        safe_action(doc, "Fechando modal após o cancelamento", lambda:
...
def main():
    global doc
    try:
        log(doc, "🚀 Iniciando teste de cadastro de CRM/Contato Externo...")
