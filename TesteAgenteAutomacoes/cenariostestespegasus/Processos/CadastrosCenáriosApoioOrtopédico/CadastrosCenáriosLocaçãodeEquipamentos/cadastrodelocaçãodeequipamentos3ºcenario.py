...
doc.add_paragraph("Cadastro de Locação de Equipamentos - Apoio Ortopédico – Cenário 3: Nesse teste será realizada a verificação de todas as mesnagens de alerta do cadastro")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_locacao_equipamentos_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Avançando para a aba: 'Adicionar Equipamentos', sem preecher campos (Verificação da mensagem de alerta)", lambda:
        ...
        safe_action(doc, "Avançando para a aba: 'Resumo', sem preecher campos (Verificação da mensagem de alerta)", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsApoioOrtopedico > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)'))).click()
        )
        encontrar_mensagem_alerta()

        safe_action(doc, "Abrindo Lov de Equipamentos", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='gsApoioOrtopedico']/div[2]/div[2]/div[2]/div/div[2]/div/div[1]/ul/li[1]/div/div/div/a"))).click(),
        ...
        safe_action(doc, "Selecionando equipamento", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//tr[td[1][normalize-space()='EQUIPAMENTO SELENIUM 10']]"))).click()
        )


        safe_action(doc, "Adicionando equipamento", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='gsApoioOrtopedico']/div[2]/div[2]/div[2]/div/div[2]/div/div[1]/ul/li[1]/div/a"))).click(),
        ...
        safe_action(doc, "Preenchendo Equipamento", lambda:
                    preencher_campo_xpath_com_retry(
                        ...
                        "EQUIPAMENTO SELENIUM 11"
                    ))

        safe_action(doc, "Pesquisando equipamento", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[19]/div[2]/div[1]/div[1]/div[3]/a"))).click(),
         

        )

        safe_action(doc, "Selecionando equipamento", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//tr[td[1][normalize-space()='EQUIPAMENTO SELENIUM 11']]"))).click()
        )
        ...
        safe_action(doc, "Selecionando equipamento", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//tr[td[1][normalize-space()='EQUIPAMENTO SELENIUM 12']]"))).click()
        )
        ...
        safe_action(doc, "Finalizando cadastro", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsApoioOrtopedico > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > div'))).click()
        )


        safe_action(doc, "Recusando geração de Documento da Locação", lambda:
