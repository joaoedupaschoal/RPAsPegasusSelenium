...
doc.add_paragraph("Cadastro de Conta Bancária – Cenário 4: Preenchimento dos campos NÃO obrigatórios e salvamento.")
...
def finalizar_relatorio():
    ...
    nome_arquivo = f"relatorio_conta_bancaria_cenario_4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    conta_debito = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "#fmod_10 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div.cat_22.categoriaHolder > div > div > div:nth-child(23) > div > div > a")))
    driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", conta_debito)
