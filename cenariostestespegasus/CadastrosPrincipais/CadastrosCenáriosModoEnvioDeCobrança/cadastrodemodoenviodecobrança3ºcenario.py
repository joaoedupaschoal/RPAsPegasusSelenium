...
doc.add_paragraph("Cadastro de Modo Envio de Cobrança – Cenário 3: Preenchimento dos campos NÃO obrigatórios e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_modo_envio_cobranca_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Abrindo menu Modo Envio de Cobrança", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("Modo Envio de Cobrança", Keys.ENTER)
    ))

    safe_action(doc, "Clicando em Cadastrar", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10075 > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span"))).click()
    ))


    safe_action(doc, "Salvando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10075 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroModoEnvioCobranca > div.btnHolder > a.btModel.btGray.btsave"
    ).click())
    ...
    safe_action(doc, "Fechando modal após salvamento", lambda: wait.until(EC.element_to_be_clickable((
        By.CSS_SELECTOR, "#fmod_10075 > div.wdTop.ui-draggable-handle > div.wdClose > a"
        ))
    ).click())
