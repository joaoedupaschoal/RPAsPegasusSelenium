...
doc.add_paragraph("Cadastro de Registro de Óbito Pet – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_registro_obito_pet_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
def cancelar_cadastro_melhorado():
    def acao():
        # Encontra o botão de salvar
        save_btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10087 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroRegistroObitoPet > div.btnHolder > a.btModel.btGray.btcancel")))
...
try:
    ...
    safe_action(doc, "Cancelndo cadastro", cancelar_cadastro_melhorado())

    # Verifica mensagem de sucesso/erro
    safe_action(doc, "Verificando resultado do cancelamento", lambda: encontrar_mensagem_alerta())
