...
doc.add_paragraph("Cadastro de Pet – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_pet_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Selecionando a Raça TESTE RAÇA SELENIUM AUTOMATIZADO", abrir_modal_e_selecionar(
        ...
        "TESTE RAÇA SELENIUM AUTOMATIZADO",
        None,  # Usa ENTER ao invés de botão
        "//td[contains(text(), 'TESTE RAÇA SELENIUM AUTOMATIZADO')]"
    ))
    ...
    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10085 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroPet > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
