...
def main():
    ...
    doc.add_paragraph("Cadastro de Cronograma de Faturamento - Cenário 2")
    doc.add_paragraph(f"🗓️ Data do teste: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
    doc.add_paragraph("Neste teste, o robô preencherá os campos obrigatórios e cancelará o cadastro de um novo Cronograma de Faturamento.")
    ...
    def finalizar_relatorio():
        doc_name = f"relatorio_cronograma_faturamento_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
        doc.save(doc_name)
    ...
    def cancelar():
        cancelar_btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,
            "#fmod_10053 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel")))
        cancelar_btn.click()
        time.sleep(2)
    ...
    sucesso, _ = safe_action("Preenchendo Exercício", preencher_exercicio)
    if not sucesso:
        finalizar_relatorio()
        return

    sucesso, _ = safe_action("Selecionando Concessionária", selecionar_concessionaria)
    if not sucesso:
        finalizar_relatorio()
        return
    ...
    # CANCELANDO CADASTRO
    sucesso, _ = safe_action("Clicando no botão Cancelar", cancelar)
    if not sucesso:
        finalizar_relatorio()
        return
    
    registrar_screenshot_unico("apos_salvar", "Clique no botão Cancelar realizado.")

    sucesso, _ = safe_action("Confirmando cancelamento", confirmar_cancelamento)
    if not sucesso:
        finalizar_relatorio()
        return
    
    registrar_screenshot_unico("apos_salvar", "Cancelamento confirmado.")
