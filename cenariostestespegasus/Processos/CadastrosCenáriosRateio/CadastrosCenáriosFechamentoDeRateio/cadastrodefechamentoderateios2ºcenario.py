...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando em Fechamento de Rateio", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsRateio > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span'))).click()
        )

        time.sleep(5)

        safe_action(doc, "Pesquisando Grupo de Rateio (Teste de disparo)", lambda: 
        ...
        time.sleep(1)
        safe_action(doc, "Fechando Consulta de Lotes", lambda: 
            wait.until(EC.element_to_be_clickable((By.XPATH, "//a[contains(@class,'fa') and contains(@class,'fa-close')]"))).click(),
        )
        time.sleep(2)

        safe_action(doc, "Realizando Fechamento sem selecionar Falecido (Teste de disparo)", lambda: 
        ...
        safe_action(doc, "Concluindo Fechamento sem preencher Data Vencimento (Teste de disparo)", lambda:
        ...
        safe_action(doc, "Reabrindo modal de Fechamento", lambda:
            force_click(driver, By.XPATH, "//a[contains(@class,'btFechamento') and normalize-space(.)='Fechamento']")
        )
        ...
        safe_action(doc, "Clicando em 'Ok (Teste de disparo)", lambda:
...
def main():
    global doc
    try:
        log(doc, "🚀 Iniciando teste de Fechamento de Rateio")
