...
doc.add_paragraph("Consulta de Registro de Chamadas - Callcenter – Cenário 1: Realização da consulta de Registro de Chamadas")
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Abrindo Registro de Chamadas", lambda: (
            aguardar_elemento('#gsCallCenter > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span').click(),
            time.sleep(3)
        ), critico=True)

        # 6. Preenchimento dos filtros
        safe_action(doc, "Selecionando Pessoa", 
                   abrir_modal_e_selecionar_robusto(
                       '#gsCallCenter > div.wdTelas > div.telaRegistroChamadas.telaConsulta.relative > div.formRow.formLastLine > div:nth-child(1) > div > a',
                       '#txtPesquisa',
                       'TESTE TITULAR 233',
        ...
        safe_action(doc, "Selecionando Pacote", 
                   abrir_modal_e_selecionar_robusto(
                       '#gsCallCenter > div.wdTelas > div.telaRegistroChamadas.telaConsulta.relative > div.formRow.formLastLine > div:nth-child(2) > div > a',
        ...
        safe_action(doc, "Preenchendo Número do Contrato Inicial", 
                   preencher_campo_robusto('#gsCallCenter > div.wdTelas > div.telaRegistroChamadas.telaConsulta.relative > div.formRow.formLastLine > div:nth-child(3) > input[type=text]', '113060'))

        safe_action(doc, "Preenchendo Número do Contrato Final", 
                   preencher_campo_robusto('#gsCallCenter > div.wdTelas > div.telaRegistroChamadas.telaConsulta.relative > div.formRow.formLastLine > div:nth-child(4) > input[type=text]', '113060'))
        ...
        safe_action(doc, "Capturando tela de dados", lambda: (
            time.sleep(3),
            take_screenshot(driver, doc, "dados_contrato_pet", forcar=True)
        ))
        ...
        safe_action(doc, "Capturando mensagens", lambda: (
            time.sleep(2),
            take_screenshot(driver, doc, "mensagens_contrato_pet", forcar=True)
        ))

        safe_action(doc, "Adicionando nova mensagem", 
                   clicar_elemento_xpath_robusto("//a[contains(@class,'btAdd') and contains(normalize-space(.),'Adicionar Mensagem')]"))
        ...
        safe_action(doc, "Abrindo Dados do Titular", 
                   clicar_elemento_xpath_robusto("//span[contains(@class,'btTitular') and @title='Dados do Titular']"))
        ...
        safe_action(doc, "Clicando na aba 'Dados Complementares' e capturando Screenshot", 
                   clicar_elemento_xpath_robusto("//a[@class='dadosComplementares' and normalize-space(text())='Dados Complementares']"),
                   )


        safe_action(doc, "Clicando na aba 'Endereços da Pessoa' e capturando Screenshot'", 
                   clicar_elemento_xpath_robusto("//a[@class='enderecos' and normalize-space(text())='Endereços da Pessoa']"),
                   )

        safe_action(doc, "Salvando Titular", 
        ...
        safe_action(doc, "Clicando na aba 'Itens Locados' e capturando Screenshot", 
                   clicar_elemento_xpath_robusto("//a[@ref='itensLocados' and normalize-space(text())='Itens Locados']"),
                   )
        ...
        safe_action(doc, "Clicando em Adicionar Contato", 
                   clicar_elemento_xpath_robusto("//span[contains(@class,'btEdit') and @title='Adicionar contato']"))


        safe_action(doc, "Preenchendo registro de contato", 
            preencher_textarea_por_indice(0,
                 "TESTE DESCRIÇÃO DO CONTATO SELENIUM AUTOMATIZADO (Automação de Testes): Teste de contato em registro de chamadas."))

        safe_action(doc, "Salvando Contato", 
                   clicar_elemento_xpath_robusto("//button[contains(@class,'btGray') and contains(normalize-space(.),'Salvar')]"),
                   )

        log(doc, "🔍 Verificando mensagens de alerta após salvar Contato...")
        encontrar_mensagem_alerta()
...
def main():
    ...
    try:
        log(doc, "🚀 INICIANDO TESTE DE CONSULTA DE REGISTRO DE CHAMADAS DO CALLCENTER")
        log(doc, "="*60)
