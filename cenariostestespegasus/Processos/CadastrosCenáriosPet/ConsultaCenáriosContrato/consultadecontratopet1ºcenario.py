...
doc.add_paragraph("Consulta de Contrato Pet – Cenário 1: Realização da consulta de contrato Pet")
...
def finalizar_relatorio():
    ...
    try:
        nome_arquivo = f"relatorio_consulta_contrato_pet_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
def executar_teste():
    ...
    try:
        ...
        log(doc, "🎯 Iniciando execução do teste de consulta de contrato Pet")
        ...
        safe_action(doc, "Abrindo consulta de contrato", lambda: (
            aguardar_elemento('#gsPet > div.wdTelas > div > ul > li:nth-child(2) > a > span').click(),
            time.sleep(3)
        ), critico=True)

        # 6. Preenchimento dos filtros
        safe_action(doc, "Selecionando Pessoas Envolvidas", 
                   abrir_modal_e_selecionar_robusto(
                       '#gsPet > div.wdTelas > div.telaConsulta.telaConsultaContratoPet > div > div.formRow.filtrosConsulta > div:nth-child(1) > div > a',
                       '#txtPesquisa',
                       'TESTE TUTOR SELENIUM AUTOMATIZADO',
                       'body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(2) > a',
                       "//td[contains(text(), 'TESTE TUTOR SELENIUM AUTOMATIZADO')]"
                   ))

        safe_action(doc, "Preenchendo Número do Contrato", 
                   preencher_campo_robusto('#gsPet > div.wdTelas > div.telaConsulta.telaConsultaContratoPet > div > div.formRow.filtrosConsulta > div:nth-child(2) > input', '113079'))

        safe_action(doc, "Preenchendo Data Inicial Cadastro", 
                   preencher_datepicker_por_indice(0, "13/08/2025"))

        safe_action(doc, "Preenchendo Data Final Cadastro", 
                   preencher_datepicker_por_indice(1, "13/08/2025"))
        
        safe_action(doc, "Preenchendo Vigência Inicial", 
                   preencher_datepicker_por_indice(2, "13/08/2025"))

        safe_action(doc, "Preenchendo Vigência Final", 
                   preencher_datepicker_por_indice(3, "13/11/2025"))

        safe_action(doc, "Selecionando Pacote Pet", 
                   abrir_modal_e_selecionar_robusto(
                       '#gsPet > div.wdTelas > div.telaConsulta.telaConsultaContratoPet > div > div.formRow.filtrosConsulta > div:nth-child(7) > div > a',
                       '#txtPesquisa',
                       'PACOTE PET TESTE SELENIUM AUTOMATIZADO',
                       'body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(2) > a',
                       "//td[contains(text(), 'PACOTE PET TESTE SELENIUM AUTOMATIZADO')]"
                   ))
        ...
        if not resultado_validacao or not resultado_validacao.get('encontrou_registros', False):
            ...
            safe_action(doc, "Fechando tela (sem registros)", 
                       clicar_elemento_robusto("#gsPet > div.wdTop.ui-draggable-handle > div > a"))
            return True
        ...
        # 10. Visualização de detalhes
        safe_action(doc, "Visualizando detalhes do Contrato Pet", 
                   clicar_elemento_xpath_robusto('//*[@id="DataTables_Table_2"]/tbody/tr/td[7]/span[1]'))

        safe_action(doc, "Capturando tela de detalhes", lambda: (
            time.sleep(3),
            take_screenshot(driver, doc, "detalhes_contrato_pet", forcar=True)
        ))

        # 11. Mensagens do contrato
        safe_action(doc, "Abrindo mensagens do Contrato Pet", 
                   clicar_elemento_robusto("body > div.modalHolder > div.modal.overflow.fullscreen.bgBlur > div.detalhesContratoPet > div > div > div.aba.detalhes > div.row > div.col-sm-4.rAlign > button:nth-child(1)"))
...
def main():
    ...
    try:
        log(doc, "🚀 INICIANDO TESTE DE CONSULTA DE CONTRATO PET")
        log(doc, "="*60)
