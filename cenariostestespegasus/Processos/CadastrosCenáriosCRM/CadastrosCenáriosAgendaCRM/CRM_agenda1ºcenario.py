...
doc.add_paragraph("Agenda CRM – Cenário 1: Consulta teste de Eventos na Agenda CRM")
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando em Agenda CRM", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCrm > div.wdTelas > div > ul > li:nth-child(3) > a > span'))).click()
        )

        time.sleep(5)

        safe_action(doc, "Abrindo o Lov de Contatos", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'body > div.DESKTOP.AGENDA.AGENDA_CRM.clearfix > div.filters.overflow.overflowY > div > div:nth-child(2) > div > a'))).click()
        )

        time.sleep(2)

        safe_action(doc, "Selecionando Filtro como: Número CRM", lambda:
            Select(wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(1) > select")))).select_by_visible_text("Número CRM")
        )

        safe_action(doc, "Preechendo Número CRM", lambda:
            wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div.formCol.divPesquisa > input.codigoPesquisa"))).send_keys("60") 
        )
        ...
        safe_action(doc, "Selecionando Contato", lambda: (
            time.sleep(2),
            wait.until(EC.element_to_be_clickable((By.XPATH, "//td[contains(text(), '60')]"))).click()
        ))



        safe_action(doc, "Abrindo o Lov de Usuário", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, 'body > div.DESKTOP.AGENDA.AGENDA_CRM.clearfix > div.filters.overflow.overflowY > div > div:nth-child(3) > div > a'))).click()
        )



        safe_action(doc, "Preechendo Nome do Usuário", lambda:
            wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#txtPesquisa"))).send_keys("JOÃO EDUARDO")
        )
        time.sleep(1)

        safe_action(doc, "Pesquisando:", lambda:
            clicar_elemento_robusto(driver, wait, 'body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div:nth-child(2) > a')
        )


        safe_action(doc, "Clicando no checkbox", lambda: (
            time.sleep(2),
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#DataTables_Table_2 > tbody > tr > td:nth-child(1) > input[type=checkbox]"))).click()
        ))

        safe_action(doc, "Selecionando Usuário:", lambda: (
            time.sleep(2),
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#lov-crm-usuario > a"))).click()
        ))

        safe_action(doc, "Buscando Agenda:", lambda: (
            time.sleep(2),
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "body > div.DESKTOP.AGENDA.AGENDA_CRM.clearfix > div.filters.overflow.overflowY > div > div:nth-child(4) > a"))).click()
        ))

        safe_action(doc, "Clicando em algum evento:", lambda: (
            time.sleep(2),
            wait.until(EC.element_to_be_clickable((By.XPATH, "//a[contains(@class, 'fc-time-grid-event') and contains(.//div[@class='fc-title'], 'TESTE TITULAR 233')]"))).click()
        ))

        time.sleep(3)

        safe_action(doc, "Screenshot do evento", lambda: (
            take_screenshot(driver, doc, "evento_selecionado"),
            log(doc, "📸 Screenshot do evento selecionado tirada.")
        ))


        safe_action(doc, "Fechando modal do evento:", lambda:
            clicar_elemento_robusto(driver, wait, 'body > div.modalHolder > div.modal.overflow > a')
        )


        safe_action(doc, "Fechando modal da Agenda CRM", lambda:
            clicar_elemento_robusto(driver, wait, 'body > div.DESKTOP.AGENDA.AGENDA_CRM.clearfix > div.header > h2 > a')
        )

        safe_action(doc, "Fechando modal CRM", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTop.ui-draggable-handle > div > a')
        )
...
def main():
    global doc
    try:
        log(doc, "🚀 Iniciando teste de consulta da Agenda/CRM")
