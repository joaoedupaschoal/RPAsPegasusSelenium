...
doc.add_paragraph("Cadastro de Contato Externo (CRM) – Cenário 1: Preenchimento completo e salvamento.")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_contato_externo_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Finalizando cadastro", lambda:
            clicar_elemento_robusto(driver, wait, "#gsCrm > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > div")
        )
