...
doc.add_paragraph("Funil de Vendas Pipeline (CRM) – Cenário 1: Consulta teste no Funil de Vendas Pipeline.")
...
dados_funil_capturados = {
    'leads_contatos': 'N/A',
    'apresentacao': 'N/A',
    'proposta_enviada': 'N/A'
}
...
def finalizar_relatorio():
    global driver, doc, dados_funil_capturados
    
    # Adiciona as informações do funil ao relatório DEPOIS que o navegador fechar
    if any(valor != 'N/A' for valor in dados_funil_capturados.values()):
        doc.add_heading("Informações do Funil de Vendas Pipeline:", level=2)
        log(doc, f"Leads/Contatos feitos: {dados_funil_capturados['leads_contatos']}")
        log(doc, f"Apresentação: {dados_funil_capturados['apresentacao']}")
        log(doc, f"Proposta enviada: {dados_funil_capturados['proposta_enviada']}")
    
    nome_arquivo = f"relatorio_funil_de_vendas_pipeline_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
# ==== FUNÇÃO PARA CAPTURAR DADOS DO FUNIL ====
def capturar_dados_funil():
    """Captura os dados do funil de vendas após a consulta"""
    global driver, doc, dados_funil_capturados
    ...
    try:
        ...
        # Tenta capturar os elementos com diferentes seletores possíveis
        try:
            # Tenta o seletor corrigido (removido o ponto extra)
            element = driver.find_element(By.CSS_SELECTOR, "#funil-vendas-base > div > div.funil-vendas-conteudo.qtd > span")
            dados_funil_capturados['leads_contatos'] = element.text
        except Exception as e:
            # Tenta seletores alternativos
            try:
                element = driver.find_element(By.CSS_SELECTOR, ".funil-vendas-base .qtd span")
                dados_funil_capturados['leads_contatos'] = element.text
            except:
                pass
        
        try:
            element = driver.find_element(By.CSS_SELECTOR, "#funil-vendas-meio > div > div.funil-vendas-conteudo.qtd > span")
            dados_funil_capturados['apresentacao'] = element.text
        except Exception as e:
            try:
                element = driver.find_element(By.CSS_SELECTOR, ".funil-vendas-meio .qtd span")
                dados_funil_capturados['apresentacao'] = element.text
            except:
                pass
        
        try:
            element = driver.find_element(By.CSS_SELECTOR, "#funil-vendas-ponta > div > div.funil-vendas-conteudo.qtd > span")
            dados_funil_capturados['proposta_enviada'] = element.text
        except Exception as e:
            try:
                element = driver.find_element(By.CSS_SELECTOR, ".funil-vendas-ponta .qtd span")
                dados_funil_capturados['proposta_enviada'] = element.text
            except:
                pass
        
        return dados_funil_capturados['leads_contatos'], dados_funil_capturados['apresentacao'], dados_funil_capturados['proposta_enviada']
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Acessando CRM", lambda: (
            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});",
                                  wait.until(EC.presence_of_element_located((By.XPATH, '/html/body/div[15]/ul/li[35]/img')))),
            wait.until(EC.element_to_be_clickable((By.XPATH, '/html/body/div[15]/ul/li[35]/img'))).click()
        ))

        safe_action(doc, "Clicando em Funil de Vendas/Pipeline", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCrm > div.wdTelas > div > ul > li:nth-child(2) > a > span'))).click()
        )

        safe_action(doc, "Preenchendo Data Início", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "(//input[contains(@class,'hasDatepicker dataInicial mandatory') and @type='text' and contains(@style,'width: 100px;')])[1]",
                hoje
            )
        )

        safe_action(doc, "Preenchendo Data Fim", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "(//input[contains(@class,'hasDatepicker dataFinal mandatory') and @type='text' and contains(@style,'width: 100px;')])[1]",
                hoje
            )
        )

        safe_action(doc, "Clicando no botão de pesquisa", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTelas > div.crmFunilVendas > div > div.formRow.filtrosConsulta > div:nth-child(4) > a')
        )

        safe_action(doc, "Screenshot do Funil de Vendas/Pipeline", lambda: take_screenshot(driver, doc, "screenshot_funil_vendas"))

        # Captura os dados do funil APÓS a consulta ser executada
        capturar_dados_funil()

        safe_action(doc, "Fechando modal após a Consulta", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCrm > div.wdTop.ui-draggable-handle > div.wdClose > a')
        )
...
# ==== MAIN ====
def main():
    global doc, driver, dados_funil_capturados
    
    try:
        log(doc, "🚀 Iniciando teste de consulta do Funil de Vendas Pipeline...")
        sucesso = executar_teste()
        
        # Fechar o navegador ANTES de finalizar o relatório
        if driver:
            try:
                driver.quit()
                driver = None  # Garantir que não será usado novamente
            except:
                pass

        # Adiciona alguns parágrafos vazios para espaçamento
        for _ in range(3):
            doc.add_paragraph()

        # Adiciona o título da seção de resultados
        doc.add_heading("INFORMAÇÕES DO FUNIL DE VENDAS PIPELINE:", level=2)

        # Verifica se conseguiu capturar algum dado válido
        dados_validos = {k: v for k, v in dados_funil_capturados.items() if v != 'N/A'}
        
        if dados_validos:
            # Cria a tabela com os dados capturados
            tabela = doc.add_table(rows=1, cols=2)
            tabela.cell(0, 0).text = "Etapa"
            tabela.cell(0, 1).text = "Valor"

            # Mapeia nomes mais legíveis para as etapas
            nomes_etapas = {
                'leads_contatos': 'Leads/Contatos',
                'apresentacao': 'Apresentação',
                'proposta_enviada': 'Proposta Enviada'
            }

            # Adiciona uma linha para cada dado válido
            for etapa, valor in dados_validos.items():
                linha = tabela.add_row()
                linha.cells[0].text = nomes_etapas.get(etapa, etapa.title())
                linha.cells[1].text = str(valor)
        else:
            # Se não conseguiu capturar dados, informa no relatório
            doc.add_paragraph("⚠️ Não foi possível capturar os dados do funil de vendas.")
            doc.add_paragraph("Verifique se a consulta retornou resultados e se os seletores estão corretos.")

        # Adiciona informação sobre o resultado do teste
        if sucesso:
        ...
        # Salva o relatório
        nome_arquivo = f"relatorio_cadastro_funil_de_vendas_pipeline_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
        try:
            doc.save(nome_arquivo)
