...
doc.add_paragraph("Cadastro de Agenda de Ambulância – Cenário 1: Preenchimento completo e salvamento.")
...
def finalizar_relatorio():
    ...
    nome_arquivo = f"relatorio_agenda_ambulancia_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        xpath = "//tr[td[1][text()='TESTE TITULAR AMBULANCIA'] and td[2][text()='Física'] and td[3][text()='431.055.280-32'] and td[7][text()='30/01/1999']]"

        safe_action(doc, "Clicando na linha do 'TESTE TITULAR AMBULANCIA'", lambda: WebDriverWait(driver, 10).until(
        ...
        safe_action(doc, "Preenchendo Telefone", preencher_campo_com_retry( driver, wait,   
        ...
        safe_action(doc, "Selecionando Motorista", lambda: WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//td[normalize-space(text())='MOTORISTA AMBULANCIA SELENIUM']"))
        ).click())
        ...
        safe_action(doc, "Finalizando cadastro", clicar_elemento_robusto(
            '#gsAgendaAmbulancia > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > div'
        ))
