...
doc.add_paragraph("Cadastro de Cotação - Gestor de Compras – Cenário 3: Preenchimento completo e envio de E-mail")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_de_cotacao_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Preenchendo Condição de Pagamento", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "//input[contains(@class,'formaPagamento')]",
                "100"
            )
        )
        ...
        safe_action(doc, "Preenchendo Forma de Pagamento", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait,
                "//input[contains(@class,'tipoPagamento')]",
                "Teste"
            )
        )
        ...
        safe_action(doc, "Enviando E-mail", lambda:
            driver.execute_script(
                "arguments[0].click();",
                driver.find_element(By.XPATH, "//a[@class='btModel btGray fRight' and normalize-space()='Enviar E-mail']")
            )
        )
        time.sleep(2)
