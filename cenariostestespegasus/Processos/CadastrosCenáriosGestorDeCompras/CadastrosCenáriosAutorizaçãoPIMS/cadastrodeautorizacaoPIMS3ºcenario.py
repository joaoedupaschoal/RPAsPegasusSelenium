...
doc.add_paragraph("Autorização PIMS - Gestor de Compras – Cenário 3: Preenchimento completo e envio ao Almoxerifado")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_autorizacao_de_PIMS_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        def clicar_primeira_filial(driver, wait, nome="Filial Gold 1"):
            """
            Clica apenas na primeira <li> que contenha o texto informado.
            Ideal para listas de filiais, lojas ou seletores simples.
            """
            try:
                elemento = wait.until(
                    EC.element_to_be_clickable(
                        (By.XPATH, f"(//li[normalize-space(text())='{nome}'])[1]")
                    )
                )

                driver.execute_script("arguments[0].scrollIntoView(true);", elemento)
                driver.execute_script("arguments[0].click();", elemento)
                print(f"✅ Primeira filial '{nome}' clicada com sucesso.")
                return True

            except Exception as e:
                print(f"❌ Erro ao clicar na primeira filial '{nome}': {e}")
                return False
        ...
        safe_action(doc, "Clicando em Autorização de PIMS", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(3) > a > span'))).click()
        )

        time.sleep(5)


        safe_action(doc, "Preenchendo Data inicial", lambda: preencher_campo_xpath_com_retry(driver, wait, "//input[@type='text' and @class='hasDatepicker dataI' and @maxlength='10' and @style='width: 100px;' and @grupo='']", "07/02/2025"))
        ...
        safe_action(doc, "Preenchendo Observações", preencher_textarea_por_indice(0, "TESTE OBSERVAÇÃO SELENIUM AUTOMATIZADO (Automação de Testes): Teste de Observação longa para validar o campo textarea no sistema.")())


        safe_action(doc, "Enviando ao Almoxerifado", lambda:
            driver.execute_script(
                "arguments[0].click();",
                driver.find_element(By.XPATH, "//a[.//span[contains(@class, 'sp-retiradaAlmoxarifado')] or contains(., 'Almoxarifado')]")
            )
        )
        time.sleep(1)
        safe_action(doc, "Clicando na primeira filial", lambda:
        clicar_primeira_filial(driver, wait, "Filial Gold 1"))




        safe_action(doc, "Fechando Modal de Autorização de PIMS", lambda:
            driver.execute_script(
                "arguments[0].click();",
