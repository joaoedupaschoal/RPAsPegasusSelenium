...
doc.add_paragraph("Autorização PIMS - Gestor de Compras – Cenário 1: Preenchimento completo e Autorização")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_autorizacao_de_PIMS_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando no botão Autorizar", lambda:
            driver.execute_script(
                "arguments[0].click();",
                driver.find_element(By.XPATH, "//a[span[contains(@class,'sp-confirmarCaixa')] and contains(normalize-space(.), 'Autorizar')]")
            )
        )
        time.sleep(1)
