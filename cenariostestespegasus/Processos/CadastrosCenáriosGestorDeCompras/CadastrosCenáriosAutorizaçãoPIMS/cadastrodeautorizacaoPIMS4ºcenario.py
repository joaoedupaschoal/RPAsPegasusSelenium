...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_autorizacao_de_PIMS_cenario_4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando no botão Pedido Direto", lambda:
            driver.execute_script(
                "arguments[0].click();",
                driver.find_element(By.XPATH, "//a[.//span[contains(@class, 'sp-pedidoDireto')] or contains(., 'Pedido Direto')]")
            )
        )
        time.sleep(1)
        ...
        safe_action(doc, "Preenchendo Valor do frete", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait, "//input[contains(@class, 'frete')]",
                "50,00"
            )
        )
        ...
        safe_action(doc, "Finalizando", lambda:
            driver.execute_script(
                "arguments[0].click();",
                driver.find_element(By.XPATH, "//a[contains(@class, 'btGreen') and contains(., 'Finalizar')]")
            )
        )
        time.sleep(2)
