...
doc.add_paragraph("Cadastro de PIMS - Gestor de Compras – Cenário 3: Não preencherá a aba Informações PIMS, para verificar se o sistema está disparando a mensagem de alerta corretamente.")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_de_PIMS_cenario_3_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Tentando avançar para a aba seguinte (para o disparo a mensagem)", lambda:
            clicar_elemento_robusto(driver, wait, '#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)')
        )
