...
doc.add_paragraph("Cadastro de PIMS - Gestor de Compras – Cenário 2: Preenchimento completo e cancelamento")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_de_PIMS_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando em Cadastro de PIMS", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span'))).click()
        )

        time.sleep(5)

        safe_action(doc, "Selecionando Filial", selecionar_opcao(
            "#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.step1 > div > div:nth-child(1) > div:nth-child(1) > select",
            "TESTE FILIAL SELENIUM"
        ))

        safe_action(doc, "Selecionando Departamento", selecionar_opcao(
            "#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.step1 > div > div:nth-child(1) > div:nth-child(2) > select",
            "Teste"
        ))

        safe_action(doc, "Selecionando Centro de Custo", selecionar_opcao(
            "#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.wizardHolder > div > div.step1 > div > div:nth-child(1) > div:nth-child(3) > select",
            "CENTRO DE CUSTO JOÃO"
        ))

        safe_action(doc, "Preenchendo Motivo da solicitação", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait, "//*[@id='gsCompras']/div[2]/div[2]/div[2]/div/div[1]/div/div[3]/div/textarea",
                "TESTE MOTIVO DA SOLICITAÇÃO - TEXTO LONGO PRA PREENCHER A TEXTAREA COM O MÁXIMO DE CARACTERES POSSÍVEIS QUE É x CARACTERES. VAMOS LÁ, VAMOS LÁ, VAMOS LÁ, VAMOS LÁ, VAMOS LÁ TESTE MOTIVO DA SOLICITAÇÃO - TEXTO LONGO PRA PREENCHER A TEXTAREA COM O MÁXIMO DE CARACTERES POSSÍVEIS QUE É x CARACTERES. VAMOS LÁ, VAMOS LÁ, VAMOS LÁ, VAMOS LÁ, VAMOS LÁ"
            )
        )

        safe_action(doc, "Avançando para a aba: 'Produtos'", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(3)'))).click()
        )

        safe_action(doc, "Abrindo Lov de Produtos", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='gsCompras']/div[2]/div[2]/div[2]/div/div[2]/div/div[1]/ul/li[1]/div/div/a"))).click()
        )
        ...
        safe_action(doc, "Pesquisando Produto", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[18]/div[2]/div[1]/div[1]/div[3]/a"))).click()
        )

        safe_action(doc, "Selecionando Produto", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "//tr[td[1][normalize-space()='BIBLIA TESTE KAUAN']]"))).click()
        )

        # ---- AQUI ERA O PONTO QUE GERAVA O ERRO (lambda com múltiplas expressões) ----
        safe_action(doc, "Avançando para a aba: 'Serviços'", avancar_para_servicos)
        # -------------------------------------------------------------------------------

        safe_action(doc, "Abrindo Lov de Serviços", abrir_lov_servicos)

        safe_action(doc, "Preenchendo Nome", lambda:
            preencher_campo_xpath_com_retry(
                driver, wait, "/html/body/div[18]/div[2]/div[1]/div[1]/div[1]/input",
                "EXUMAÇÃO JOÃO TESTE"
            )
        )

        safe_action(doc, "Pesquisando Serviço", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[18]/div[2]/div[1]/div[1]/div[2]/a"))).click()
        )

        safe_action(doc, "Selecionando Serviço", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "//tr[td[1][normalize-space()='EXUMAÇÃO JOÃO TESTE']]"))).click()
        )
        ...
        safe_action(doc, "Cancelando cadastro", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > a:nth-child(1)'))).click()
        )
...
def main():
    global doc
    try:
        log(doc, "🚀 Iniciando teste de Cadastro de PIMS")
