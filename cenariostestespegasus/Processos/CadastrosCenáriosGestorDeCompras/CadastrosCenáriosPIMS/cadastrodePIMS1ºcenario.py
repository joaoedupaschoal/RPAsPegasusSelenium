...
doc.add_paragraph("Cadastro de PIMS - Gestor de Compras – Cenário 1: Preenchimento completo e salvamento")
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_cadastro_de_PIMS_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Finalizando cadastro", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.wdWizard.clearfix.telaConsulta > div.btnHolder > div'))).click()
        )
