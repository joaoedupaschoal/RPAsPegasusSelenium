...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_autorizacao_de_pedido_de_compra_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
    ...
    try:
        ...
        safe_action(doc, "Clicando em Negar Pedido", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//a[contains(@class,'sprites sp-nega') and @title='Negar pedido']"
                )))
            )
        )
