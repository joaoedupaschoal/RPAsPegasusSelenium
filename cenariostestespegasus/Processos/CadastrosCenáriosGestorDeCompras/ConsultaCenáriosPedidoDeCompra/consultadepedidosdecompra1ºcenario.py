...
doc.add_paragraph("Pedido de Compra - Gestor de Compras – Cenário 1: Rotina completa e cancelamento do Pedido")
...
# logo após inicializar driver/wait
click_xpath = lambda xp: clicar_elemento_robusto_xpath(driver, wait, xp)
...
def clicar_proxima_ate_cancelar(driver, wait, doc):
    """
    Clica no botão 'Próxima' até encontrar o ícone de Cancelar Pedido.
    Para quando o botão de cancelar for detectado ou não houver mais 'Próxima'.
    """
    try:
        while True:
            # Verifica se o botão Cancelar já está visível
            try:
                cancelar = driver.find_element(By.XPATH, "//span[@class='sprites sp-cancelar' and @title='Cancelar Pedido']")
                if cancelar.is_displayed():
                    log(doc, "✅ Botão 'Cancelar Pedido' detectado, parando paginação.")
                    return True
            ...
            try:
                botao_proxima = wait.until(
                    EC.element_to_be_clickable((By.XPATH, "//a[@id='DataTables_Table_1_next' and contains(@class,'next') and text()='Próxima']"))
                )
                if "disabled" in botao_proxima.get_attribute("class"):
                    log(doc, "⚠️ Botão 'Próxima' está desativado — nenhuma página restante.")
                    break
                ...
                log(doc, "➡️ Clicando em 'Próxima' para carregar mais registros...")
                time.sleep(2.5)  # espera carregar os novos dados
            except TimeoutException:
                log(doc, "⚠️ Botão 'Próxima' não encontrado — encerrando busca.")
                break

        log(doc, "❌ Botão 'Cancelar Pedido' não foi encontrado após todas as páginas.")
        return False

    except Exception as e:
        log(doc, f"❌ Erro ao paginar até o botão de cancelar: {e}")
        return False
...
def finalizar_relatorio():
    global driver, doc
    nome_arquivo = f"relatorio_pedido_de_compra_cenario_1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
def clicar_elemento_robusto_xpath(driver, wait, seletor_xpath, timeout=10):
    global doc
    try:
        elem = wait.until(EC.presence_of_element_located((By.XPATH, seletor_xpath)))
        try:
            driver.execute_script("""
                document.querySelectorAll('.modal, .overlay, .blockUI, .toast, .tooltip, [role="dialog"], [data-overlay]')
                .forEach(e => { if (getComputedStyle(e).position === 'fixed') e.style.display = 'none'; });
        ...
        time.sleep(0.2)
        try:
            elem = wait.until(EC.element_to_be_clickable((By.XPATH, seletor_xpath)))
            elem.click()
            return True
        except (TimeoutException, ElementClickInterceptedException, StaleElementReferenceException):
            pass
        try:
            elem = driver.find_element(By.XPATH, seletor_xpath)
            ActionChains(driver).move_to_element(elem).pause(0.05).click().perform()
            return True
        except Exception:
            pass
        ...
        try:
            ...
            pass
        log(doc, f"❌ Não foi possível clicar em: {seletor_xpath}")
        return False
    except Exception as e:
        log(doc, f"❌ Erro ao clicar robusto: {e}")
        return False
...
def executar_teste():
    ...
    try:
        ...
        safe_action(doc, "Clicando em Pedido de Compra", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(6) > a > span'))).click()
        )
        ...
        safe_action(doc, "Preenchendo Data Pedido Inicial", 
                   preencher_datepicker_por_indice(0, "09/03/2025"))
        ...
        safe_action(doc, "Pesquisando...", lambda:
            wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@class='btModel btGray btPesquisar fRight' and normalize-space(text())='Pesquisar']"))).click()
        )
        time.sleep(5)
        ...
        safe_action(doc, "Realizando consulta", lambda:
            wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#gsCompras > div.wdTelas > div.telaPedidoCompra.telaConsulta.clearfix > div.formRow.formLastLine > div:nth-child(5) > a'))).click()
        )
        time.sleep(10)
        ...
        safe_action(doc, "Paginar até encontrar Pedidos de compra a Cancelar", lambda:
            clicar_proxima_ate_cancelar(driver, wait, doc)
        )
        ...
        safe_action(doc, "Abrindo Detalhes do Pedido e Capturando screenshot", lambda: (
            driver.execute_script(
                ...
                wait.until(EC.element_to_be_clickable((
                    By.XPATH,
                    "//span[@class='sprites sp-downDocs' and @title='Detalhes']"
                )))
            ),
            wait.until(EC.visibility_of_element_located((
                By.XPATH,
                "//div[contains(@class,'modal overflow')]"  # ajuste conforme o seletor da área de detalhes
            )))
        ))

        safe_action(doc, "Fechando", lambda: (
            time.sleep(10),
            driver.execute_script(
                "arguments[0].click();",
        ...
        safe_action(doc, "Enviando por E-mail", lambda:
            driver.execute_script(
                "arguments[0].click();",
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//span[@class='sprites sp-email' and @title='Enviar por email']"
                )))
            )
        )
        ...
        safe_action(doc, "Abrindo digitalização de documentos", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//span[contains(@class,'sp-scanner') and contains(@class,'sprites') and @title='Digitalizar Documentos']"
                )))
            )
        )
        ...
        safe_action(doc, "Adicionando novo documento", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//span[contains(@class, 'sp-addFile')]"
                )))
            )
        )
        ...
        if os.path.exists(CAMINHO_ARQUIVO_UPLOAD):
            ...
            log(doc, f"Arquivo não encontrado: {CAMINHO_ARQUIVO_UPLOAD}, pulando upload")  # <— sem 'WARN'
        ...
        safe_action(doc, "Salvando arquivo digitalizado", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//a[contains(@class,'btModel') and contains(@class,'btGreen') and normalize-space(.)='Salvar']"
                )))
            )
        )
        ...
        safe_action(doc, "Fechando modal de digitalização", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "(//a[contains(@class,'fa-close') and not(contains(@style,'display:none'))])[1]"
                    
                )))
            )
        )

        safe_action(doc, "Clicando em Cancelar Pedido", lambda:
            driver.execute_script(
                "arguments[0].click();",
                wait.until(EC.presence_of_element_located((
                    By.XPATH,
                    "//span[@class='sprites sp-cancelar' and @title='Cancelar Pedido']"
                )))
            )
        )
        ...
        safe_action(doc, "Fechando modal Gestor de Compras", lambda:
            driver.execute_script(
                ...
                wait.until(EC.presence_of_element_located((
                    By.CSS_SELECTOR,
                    "#gsCompras > div.wdTop.ui-draggable-handle > div > a"
                )))
            )
        )
