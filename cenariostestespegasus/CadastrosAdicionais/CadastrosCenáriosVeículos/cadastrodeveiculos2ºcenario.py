...
doc.add_paragraph("Cadastro de Veículos - Cenário 2: Preenchimento completo e cancelamento")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_veiculo_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preenchendo o Nome do Veículo", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10004 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroVeiculo > div.catWrapper > div > div.cat_categoriaDadosVeiculo.categoriaHolder > div > div > div > div:nth-child(2) > input"))).send_keys('TESTE VEÍCULO SELENIUM AUTOMATIZADO')
    ))
    ...
    safe_action(doc, "Preenchendo o campo Placa", lambda: 
        preencher_campo_com_retry(
            driver, wait,
    ...
    safe_action(doc, "Preenchendo o campo Km Atual", lambda: 
        preencher_campo_com_retry(
            driver, wait,
    ...
    safe_action(doc, "Preenchendo o campo Notificar faltando (km)", lambda: 
        preencher_campo_com_retry(
            driver, wait,
            "#fmod_10004 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroVeiculo > div.catWrapper > div > div.cat_categoriaManutencaoPreventiva.categoriaHolder > div.groupHolder.clearfix.grupo_grupoNotificacaoManutencaoPreventiva > div > div > div:nth-child(3) > input",
            dados_veiculo['notificar_km']
        )
    )

    safe_action(doc, "Preenchendo o campo Notificar faltando (dias)", lambda: 
        preencher_campo_com_retry(
            driver, wait,
            ...
            dados_veiculo['notificar_dias']
        )
    )

    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10004 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroVeiculo > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
