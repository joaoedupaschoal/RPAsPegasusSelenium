...
doc.add_paragraph("Cadastro de Tipo de Mensalidade – Cenário 4: Preenchimento dos campos NÃO obrigatórios e salvamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_tipo_mensalidade_cenario_4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Salvando cadastro", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10027 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btsave"))).click(),
        time.sleep(1)
    ))
