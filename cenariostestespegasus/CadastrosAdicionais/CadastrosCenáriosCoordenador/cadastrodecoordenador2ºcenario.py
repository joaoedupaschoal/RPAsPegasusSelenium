...
# Refatorado e organizado: cadastrodecoordenador1ºcenario.py
...
doc.add_paragraph("Cadastro de Coordenador – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_coordenador_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    doc.save(nome_arquivo)
    log(doc, f"📄 Relatório salvo como: {nome_arquivo}")
    subprocess.run(["start", "winword", nome_arquivo], shell=True)
    driver.quit()

def gerar_dados_coordenador():
    """Gera dados fictícios para o cadastro de coordenador."""
    nome_completo = "COORDENADOR TESTE SELENIUM AUTOMATIZADO"
    ...
    profissao = fake.job()
    comissao_porcentagem = fake.random_int(min=10, max=500)
    comissao_reais = fake.random_int(min=10, max=500)
...
(nome_completo, cpf_valido, data_nascimento, data_expedicao, email, 
 telefone1, telefone2, telefone3, cidade_nascimento, pais_nascimento, 
 nome_pai, nome_mae, profissao, comissao_porcentagem, comissao_reais) = gerar_dados_coordenador()
...
try:
    ...
    safe_action(doc, "Abrindo menu Coordenador", lambda: (
        driver.find_element(By.TAG_NAME, "body").send_keys(Keys.F2),
        time.sleep(1),
        wait.until(EC.visibility_of_element_located((By.XPATH, "//input[@placeholder='Busque um cadastro']"))).send_keys("Coordenador", Keys.ENTER),
        time.sleep(3)
    ))

    safe_action(doc, "Clicando em Cadastrar", lambda: (
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10017 > div.wdTelas > div.telaInicial.clearfix.overflow.overflowY > ul > li:nth-child(1) > a > span"))).click(),
        time.sleep(2)
    ))

    safe_action(doc, "Abrindo LOV Pessoa para Coordenador", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#fmod_10017 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(2) > div > a'))).click()
    ))

    safe_action(doc, "Clicando em Novo Registro Pessoa", lambda: (
    ...
    # Preenchendo dados pessoais na modal de pessoa
    ...
    safe_action(doc, "Cancelando dados da pessoa", lambda: (
    ...
    safe_action(doc, "Preenchendo Comissão Porcentagem", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#fmod_10017 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(3) > input'))).send_keys(str(comissao_porcentagem))
    ))

    safe_action(doc, "Preenchendo Comissão Reais", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '#fmod_10017 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div:nth-child(4) > input'))).send_keys(str(comissao_reais)),
        time.sleep(5)
    ))

    safe_action(doc, "Cancelando cadastro do coordenador", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10017 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"))).click(),
        time.sleep(1)
    ))
    ...
    safe_action(doc, "Fechando modal após cancelamento", lambda: (
        wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#fmod_10017 > div.wdTop.ui-draggable-handle > div.wdClose > a"))).click(),
        time.sleep(1)
    ))
