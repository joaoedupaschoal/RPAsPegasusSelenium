...
doc.add_paragraph("Cadastro de Grupos – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_grupo_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
...
try:
    ...
    safe_action(doc, "Preechendo o Nome da Grupo", lambda: (
        time.sleep(2),
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, "#fmod_10024 > div.wdTelas > div.telaCadastro.clearfix > div.catWrapper > div > div > div > div > div > input"))
        ).send_keys('TESTE GRUPO SELENIUM AUTOMATIZADO '  + str(fake.random_int(min=1, max=1000)))
    ))

    safe_action(doc, "Cancelando cadastro", lambda: driver.find_element(
        By.CSS_SELECTOR, "#fmod_10024 > div.wdTelas > div.telaCadastro.clearfix > div.btnHolder > a.btModel.btGray.btcancel"
    ).click())
    ...
    safe_action(doc, "Fechando modal após cancelamento", lambda: wait.until(EC.element_to_be_clickable((
        By.CSS_SELECTOR, "#fmod_10024 > div.wdTop.ui-draggable-handle > div.wdClose > a"
        ))
    ).click())
