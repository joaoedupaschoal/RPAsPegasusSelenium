...
doc.add_paragraph("Cadastro de Jazigos em Lote – Cenário 5: Nesse teste, o robô preencherá todos os dados, excedendo o limite máximo de Jazigos de uma Quadra e clicará em Salvar")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_jazigos_lote_cenario_5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
try:
    ...
    safe_action_enhanced(driver, doc, "Preenchendo o nome da Quadra no campo de pesquisa",
        lambda: safe_send_keys_enhanced(driver, "body > div.modalHolder > div.modal.overflow > div:nth-child(1) > div.formRow.formLastLine > div.formCol.divPesquisa > div:nth-child(2) > input", "TESTE QUADRA LOTADA", By.CSS_SELECTOR, clear=True))
    ...
    safe_action_enhanced(driver, doc, "Selecionando Quadra", 
        lambda: selecionar_item_tabela_melhorado(driver, "TESTE QUADRA LOTADA"))
