...
doc.add_paragraph("Cadastro de Jazigos em Lote – Cenário 2: Preenchimento completo e cancelamento.")
...
def finalizar_relatorio():
    nome_arquivo = f"relatorio_jazigos_lote_cenario_2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    try:
...
try:
    ...
    safe_action_enhanced(driver, doc, "Cancelando cadastro", 
        lambda: safe_click_enhanced(driver, "#fmod_7 > div.wdTelas > div.telaCadastro.clearfix.telaCadastroJazigo > div.btnHolder > a.btModel.btGray.btcancel"))


    # Fechar modal
    safe_action_enhanced(driver, doc, "Fechando modal após cancelamento", lambda: (
    ...
    safe_action_enhanced(driver, doc, "Verificando mensagens do sistema", lambda: (
        encontrar_mensagem_alerta(driver, doc)
    ))
